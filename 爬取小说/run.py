    ...
    while True:
        ...
        try:
            ...
            if 1 <= choice <= len(other_novels):
                selected_novel = other_novels[choice - 1]
                ...
                if ask_download_novel(selected_novel):
                    ...
                    # 用户选择不下载，询问是否继续查看其他小说
                    continue_choice = input("\n是否继续查看其他小说? (y/n): ").strip().lower()
                    if continue_choice in ['n', 'no', '否']:
                        ...
                        print("\n其他搜索到的小说:")
                        print("-" * 80)
                        for j, novel in enumerate(other_novels, 1):
                            print(f"{j}. {novel['title']}")
                            print(f"   作者: {novel['writer']}")
                            print(f"   分类: {novel['sort']}")
                            print("-" * 80)
                        continue
                ...
                while True:
                    ...
                    if choice == '1':
                        ...
                        if show_other_novels(searched_novels, selected_novel):
                            return True  # 如果用户下载了小说，则结束程序
                        else:
                            break  # 返回上一级菜单
                    elif choice == '2':
                        # 搜索新的小说，跳出内层循环继续外层搜索循环
                        break
                    elif choice == '3':
                        # 退出程序
                        print("已退出程序")
